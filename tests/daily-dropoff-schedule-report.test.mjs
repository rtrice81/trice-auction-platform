...
test("daily drop-off report is admin-only and exposes the print route", () => {
  ...
  assert.match(route, /getDailyDropoffScheduleReport\(env\.trice_auction_db, requestedDate, includeCancelled\)/);
  assert.match(route, /scheduleUrl: report\.dropoffDayId \? `\/admin\/schedule\/\$\{report\.dropoffDayId\}` : "\/admin\/schedule"/);
