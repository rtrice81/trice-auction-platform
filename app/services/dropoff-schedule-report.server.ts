...
export async function getDailyDropoffScheduleReport(db: D1Database, date: string, includeCancelled = false): Promise<DailyDropoffScheduleReport> {
  ...
  const [summary, appointments] = await Promise.all([
    db.prepare(`SELECT day.id AS dropoffDayId, day.event_name AS eventName, COALESCE(day.daily_capacity_override, day.capacity_points) AS normalCapacity,
      COALESCE(totals.scheduled, 0) AS scheduled, COALESCE(totals.checkedIn, 0) AS checkedIn,
      COALESCE(totals.completed, 0) AS completed, COALESCE(totals.noShow, 0) AS noShow,
      COALESCE(totals.lastMinuteCancelled, 0) AS lastMinuteCancelled, COALESCE(totals.waitlisted, 0) AS waitlisted
      FROM (SELECT 1) base
      LEFT JOIN dropoff_days day ON day.dropoff_date = ?
      LEFT JOIN (
        SELECT COALESCE(SUM(CASE WHEN status IN ('scheduled', 'checked_in', 'completed') THEN 1 ELSE 0 END), 0) AS scheduled,
          COALESCE(SUM(CASE WHEN status = 'checked_in' THEN 1 ELSE 0 END), 0) AS checkedIn,
          COALESCE(SUM(CASE WHEN status = 'completed' THEN 1 ELSE 0 END), 0) AS completed,
          COALESCE(SUM(CASE WHEN status = 'no_show' THEN 1 ELSE 0 END), 0) AS noShow,
          COALESCE(SUM(CASE WHEN status = 'last_minute_cancelled' THEN 1 ELSE 0 END), 0) AS lastMinuteCancelled,
          COALESCE(SUM(CASE WHEN status = 'waitlisted' THEN 1 ELSE 0 END), 0) AS waitlisted
        FROM appointments WHERE appointment_date = ?
      ) totals ON 1 = 1`).bind(date, date).first<{ dropoffDayId: number | null; eventName: string | null; normalCapacity: number | null } & DailyDropoffScheduleReport["totals"]>(),
  ...
  const totals = summary ?? { scheduled: 0, checkedIn: 0, completed: 0, noShow: 0, lastMinuteCancelled: 0, waitlisted: 0 };
  return { date, dropoffDayId: summary?.dropoffDayId ?? null, eventName: summary?.eventName ?? null, capacity: summary?.dropoffDayId ? { normal: summary.normalCapacity ?? 0, confirmed: totals.scheduled, waitlist: totals.waitlisted } : null, totals, appointments: appointments.results };
}
