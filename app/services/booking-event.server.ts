...
async function getEventCapacitySnapshot(db: D1Database, eventId: number, eventDate: string) {
  const [event, settingsResult, typesResult, areasResult, dailyUsage, confirmedAreaUsage, waitlistAreaUsage, heldDailyUsage, heldAreaUsage] = await db.batch([
    ...
    db.prepare(`SELECT COALESCE(SUM(type.capacity_points), 0) AS usedPoints FROM appointments appointment JOIN dropoff_types type ON type.id = appointment.dropoff_type_id WHERE appointment.appointment_date = ? AND appointment.status IN ('scheduled', 'checked_in', 'completed')`).bind(eventDate), db.prepare(`SELECT allocation.item_area_id AS itemAreaId, COALESCE(SUM(allocation.capacity_points), 0) AS usedPoints FROM appointment_area_allocations allocation JOIN appointments appointment ON appointment.id = allocation.appointment_id WHERE appointment.appointment_date = ? AND appointment.status IN ('scheduled', 'checked_in', 'completed') GROUP BY allocation.item_area_id`).bind(eventDate), db.prepare(`SELECT allocation.item_area_id AS itemAreaId, COALESCE(SUM(allocation.capacity_points), 0) AS usedPoints FROM appointment_area_allocations allocation JOIN appointments appointment ON appointment.id = allocation.appointment_id WHERE appointment.appointment_date = ? AND appointment.status = 'waitlisted' GROUP BY allocation.item_area_id`).bind(eventDate), db.prepare(`SELECT COALESCE(SUM(reserved_daily_intake_points),0) AS usedPoints FROM booking_holds WHERE dropoff_day_id=? AND status='active' AND expires_at>CURRENT_TIMESTAMP`).bind(eventId), db.prepare(`SELECT area.id AS itemAreaId, COALESCE(SUM(CASE area.name WHEN 'Smalls' THEN hold.reserved_smalls_points WHEN 'Large/Furniture' THEN hold.reserved_large_furniture_points WHEN 'Outdoor' THEN hold.reserved_outdoor_points ELSE 0 END),0) AS usedPoints FROM booking_holds hold JOIN item_areas area ON area.name IN ('Smalls','Large/Furniture','Outdoor') WHERE hold.dropoff_day_id=? AND hold.status='active' AND hold.expires_at>CURRENT_TIMESTAMP GROUP BY area.id`).bind(eventId),
  ]);
  const eventRow = event.results[0] as { capacityPoints: number | null; dailyCapacityOverride: number | null } | undefined; const totalCapacity = eventRow?.dailyCapacityOverride ?? eventRow?.capacityPoints ?? Number((settingsResult.results[0] as { value?: string } | undefined)?.value); const usedDaily = Number((dailyUsage.results[0] as { usedPoints?: number } | undefined)?.usedPoints ?? 0) + Number((heldDailyUsage.results[0] as { usedPoints?: number } | undefined)?.usedPoints ?? 0); const remainingDaily = Math.max(0, totalCapacity - usedDaily);
  const usedByArea = new Map((confirmedAreaUsage.results as Array<{ itemAreaId: number; usedPoints: number }>).map((usage) => [usage.itemAreaId, usage.usedPoints])); for (const usage of heldAreaUsage.results as Array<{ itemAreaId: number; usedPoints: number }>) usedByArea.set(usage.itemAreaId, (usedByArea.get(usage.itemAreaId) ?? 0) + usage.usedPoints); const waitlistedByArea = new Map((waitlistAreaUsage.results as Array<{ itemAreaId: number; usedPoints: number }>).map((usage) => [usage.itemAreaId, usage.usedPoints])); const areas = areasResult.results as Array<{ id: number; capacityPoints: number; overflowPoints: number }>; const types = typesResult.results as Array<{ capacityPoints: number }>;
