...
export function CustomerBookingForm({ appointmentDate, booking, dropoffTypes, itemAreas, isAuthenticated, turnstileSiteKey, formStartToken }: BookingFormProps) {
  ...
  return <Form method="post" className="space-y-8"><input type="hidden" name="appointmentDate" value={appointmentDate}/>{!isAuthenticated ? <PublicFormProtection siteKey={turnstileSiteKey} formStartToken={formStartToken}/> : null}
    ...
    <Button type="submit" disabled={submitting}>{submitting ? "Saving your request…" : "Request drop-off appointment"}</Button>
  </Form>;
}
