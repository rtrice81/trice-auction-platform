...
export function DropoffEventForm({
  ...
  return (
    <div className="grid gap-5 md:grid-cols-2">
      ...
      <label className="block text-sm font-semibold">Override Reservation Hold Time<select name="holdDurationMinutesOverride" defaultValue={event.holdDurationMinutesOverride ?? ""} className="mt-1 block w-full rounded border border-stone-300 bg-white p-2"><option value="">Use Booking Event Setting</option><option value="5">5 minutes</option><option value="10">10 minutes</option><option value="15">15 minutes</option><option value="20">20 minutes</option><option value="30">30 minutes</option><option value="60">60 minutes</option></select><span className="mt-1 block text-xs font-normal text-stone-500">Private / internal dates use the application default when no override is selected.</span></label>
      ...
      <section className="md:col-span-2"><h2 className="text-xl font-bold">Saved storage capacities</h2><p className="mt-1 text-sm text-stone-600">These values are saved with this event and are used for bookings on this date.</p><div className="mt-3 grid gap-4 lg:grid-cols-3">{event.areas.map((area) => <fieldset key={area.itemAreaId} className="rounded border border-stone-200 p-4"><legend className="px-1 font-semibold">{area.name}</legend><label className="mt-2 block text-sm">Normal capacity<input required name={`area-${area.itemAreaId}-capacity`} type="number" min="0" step="0.01" defaultValue={area.capacityPoints} className="mt-1 block w-full rounded border border-stone-300 p-2" /></label><label className="mt-3 block text-sm">Waitlist capacity<input required name={`area-${area.itemAreaId}-overflow`} type="number" min="0" step="0.01" defaultValue={area.overflowAllowancePoints} className="mt-1 block w-full rounded border border-stone-300 p-2" /><span className="mt-1 block text-xs font-normal text-stone-500">Appointments beyond normal capacity may be placed on the waitlist until this additional capacity is reached.</span></label></fieldset>)}</div></section>
