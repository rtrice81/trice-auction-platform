...
export function AppHeader({ user, logo }: { user: ApplicationUser | null; logo: { updatedAt: string } | null }) {
  return <header className="ta-site-header print-hide">
    ...
    <div className="ta-main-nav"><Link to="/" className="ta-brand">{logo ? <img src={`/branding/logo?v=${encodeURIComponent(logo.updatedAt)}`} alt="Trice Auctions" /> : <><strong>Trice Auctions</strong><span>Consignment &amp; estate auctions</span></>}</Link><nav aria-label="Primary navigation" className="ta-primary-nav"><Link to="/">Schedule Drop-Off</Link>{user ? <><Link to="/my-appointments">My Appointments</Link><Link to="/profile">Account</Link>{user.role === "admin" ? <Link to="/admin/schedule">Admin</Link> : null}</> : <><Link to="/login">Log In</Link><Link to="/register">Create Auction</Link></>}</nav></div>
  </header>;
}
