...
export function AppHeader({ user, logo }: { user: ApplicationUser | null; logo: { updatedAt: string } | null }) {
  return <header className="ta-site-header print-hide">
    ...
    <div className="ta-main-nav"><Link to="/" className="ta-brand">{logo ? <img src={`/branding/logo?v=${encodeURIComponent(logo.updatedAt)}`} alt="Trice Auctions" /> : <><strong>Trice Auctions</strong><span>Consignment &amp; estate auctions</span></>}</Link><nav aria-label="Primary navigation" className="ta-primary-nav"><Link to="/">Schedule Drop-Off</Link>{user ? <>{hasRole(user, "consignor") ? <Link to="/my-appointments">My Appointments</Link> : null}<Link to="/profile">Account</Link>{hasRole(user, "admin") ? <Link to="/admin/schedule">Admin</Link> : null}</> : null}</nav></div>
  </header>;
}
