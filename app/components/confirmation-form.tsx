...
export function ConfirmationForm({ action, children, className, confirmation, confirmationForSubmission, method = "post" }: ConfirmationFormProps) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const dialog = document.querySelector<HTMLElement>("[data-confirmation-dialog]");
  ...
  }}>{children}</Form>{open ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-stone-950/40 p-4" role="presentation"><section data-confirmation-dialog role="dialog" aria-modal="true" aria-labelledby="confirmation-title" aria-describedby="confirmation-description" className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h2 id="confirmation-title" className="text-xl font-bold text-stone-950">{copy.title}</h2><div id="confirmation-description" className="mt-3 text-stone-700">{copy.description}</div><div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => setOpen(false)} className="rounded-lg border border-stone-300 px-4 py-2.5 font-semibold text-stone-800">Go Back</button><button ref={confirmButtonRef} type="button" onClick={() => { bypassRef.current = true; setOpen(false); formRef.current?.requestSubmit(); }} className={copy.destructive ? "rounded-lg bg-red-700 px-4 py-2.5 font-semibold text-white hover:bg-red-800" : "rounded-lg bg-stone-900 px-4 py-2.5 font-semibold text-white"}>{copy.confirmLabel}</button></div></section></div> : null}</>;
}
