...
export default function Register({ loaderData, actionData }: Route.ComponentProps) { return <main className="mx-auto max-w-md p-8"><h1 className="text-3xl font-bold">Create your account</h1>{actionData?.error && <p role="alert">{actionData.error}</p>}<Form method="post" className="mt-6 space-y-4"><input required name="name" placeholder="Name" className="w-full border p-2"/><input required type="email" name="email" placeholder="Email" className="w-full border p-2"/><input required minLength={8} type="password" name="password" placeholder="Password" className="w-full border p-2"/><PublicFormProtection siteKey={loaderData.turnstileSiteKey} formStartToken={loaderData.formStartToken}/><button className="bg-stone-900 px-4 py-2 text-white">Register</button></Form></main>; }
