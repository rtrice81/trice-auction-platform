...
import { addCustomerPrivateNote, getCustomerPrivateNotes, getCustomerStanding, removeCustomerDropoffBan, setCustomerDropoffBan } from "../services/customer-standing.server";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  return { user, saved: new URL(request.url).searchParams.get("saved") === "1", standing: customerData[0], privateNotes: customerData[1] };
}
...
export default function EditUser({ loaderData, actionData }: Route.ComponentProps) {
  const { user, standing, privateNotes } = loaderData;
