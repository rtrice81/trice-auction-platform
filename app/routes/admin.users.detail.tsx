...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const [standing, privateNotes] = await Promise.all([
    hasRole(user, "consignor") ? getCustomerStanding(env.trice_auction_db, user.id) : null,
    getCustomerPrivateNotes(env.trice_auction_db, user.id),
  ]);
  return { user, standing, privateNotes, appointments };
}
...
export async function action({ request, params }: Route.ActionArgs) {
  ...
  const targetUser = await getManagedUser(env.trice_auction_db, customerUserId);
  if (!targetUser) return data({ ok: false as const, errors: ["User was not found."] }, { status: 404 });
  if ((intent === "ban-customer" || intent === "unban-customer") && !hasRole(targetUser, "consignor")) {
    return data({ ok: false as const, errors: ["Drop-off status is only available for consignors."] }, { status: 403 });
  }
...
export default function AdminUserDetail({ loaderData, actionData }: Route.ComponentProps) {
  ...
  const isConsignor = hasRole(user, "consignor");
  return <main className="min-h-screen bg-stone-50 text-stone-900"><div className="mx-auto max-w-3xl px-6 py-12 sm:py-16">
    ...
    <section className="mt-6 rounded-2xl border border-stone-200 bg-white p-6 shadow-sm"><h2 className="text-xl font-bold">User Information</h2><dl className="mt-5 grid gap-5 sm:grid-cols-2"><Detail label="Name" value={user.name}/><Detail label="Email / login email" value={user.email}/><Detail label="Phone" value={user.phone || "Not provided"}/><Detail label="Roles" value={user.roles.join(", ") || "None assigned"}/><Detail label="Consignor Number" value={user.consignorNumber || "Not assigned"}/><Detail label="Account status" value={user.active ? "Active" : "Inactive"}/><Detail label="Created" value={user.createdAt}/></dl></section>
    ...
    {isConsignor ? <DropOffStatusTools standing={standing}/> : null}
    <InternalNotesTools privateNotes={privateNotes}/>
  </div></main>;
}
...
function DropOffStatusTools({ standing }: { standing: Awaited<ReturnType<typeof getCustomerStanding>> }) {
