...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const customerData = user.roles.includes("consignor") ? await Promise.all([
...
export default function AdminUserDetail({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return <main className="min-h-screen bg-stone-50 text-stone-900"><div className="mx-auto max-w-3xl px-6 py-12 sm:py-16">
    ...
    <DropOffStatusTools isCustomer={user.roles.includes("consignor")} standing={standing}/>
    <InternalNotesTools isCustomer={user.roles.includes("consignor")} privateNotes={privateNotes}/>
  </div></main>;
}
