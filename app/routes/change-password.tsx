...
export default function ChangePassword({ loaderData, actionData }: Route.ComponentProps) {
  ...
  return <main className="mx-auto max-w-md p-8"><h1 className="text-3xl font-bold">Change your password</h1><p className="mt-2 text-stone-600">{loaderData.required ? "Your administrator created this account. Change the temporary password to continue." : "Choose a new password."}</p>{confirmationError ? <p className="mt-4" role="alert">{confirmationError}</p> : null}{actionData?.error ? <p className="mt-4" role="alert">{actionData.error}</p> : null}<Form method="post" className="mt-6 space-y-4" onSubmit={(event) => { const form = new FormData(event.currentTarget); if (form.get("newPassword") !== form.get("confirmNewPassword")) { event.preventDefault(); setConfirmationError("The new passwords do not match."); } else setConfirmationError(null); }}><input required type="password" name="currentPassword" placeholder="Current temporary password" className="w-full border p-2" autoComplete="current-password"/><input required minLength={8} type="password" name="newPassword" placeholder="New password" className="w-full border p-2" autoComplete="new-password"/><input required minLength={8} type="password" name="confirmNewPassword" placeholder="Confirm new password" className="w-full border p-2" autoComplete="new-password"/><button className="bg-stone-900 px-4 py-2 text-white">Save new password</button></Form></main>;
}
